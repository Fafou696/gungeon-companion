const CACHE='gungeon-companion-v2';
...
self.addEventListener('fetch',e=>{
  if(e.request.mode==='navigate'){
    ...
    e.respondWith(caches.match(e.request).then(cached=>fetch(e.request).then(r=>{const c=r.clone();caches.open(CACHE).then(x=>x.put(e.request,c));return r}).catch(()=>cached)));
  }
});
